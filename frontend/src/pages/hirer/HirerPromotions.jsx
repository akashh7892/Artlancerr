...
function Img({ src, alt, className }) {
  ...
  if (err)
    return (
      <div className={`flex items-center justify-center ${className}`} style={{ background: C.inputBg }}>
        <Film size={28} style={{ color: C.muted, opacity: 0.35 }} />
      </div>
    );
  return (
    <img src={src} alt={alt} className={className} style={{ objectFit: "cover" }} onError={() => setErr(true)} />
  );
}
...
function FieldInput({ type = "text", value, onChange, placeholder, icon }) {
  return (
    <div className="relative">
      {icon && (
        <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: C.muted }}>
          {icon}
        </div>
      )}
      <input
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          >
            <div>
              <p style={{ color: C.text, fontSize: 20, fontWeight: 600, margin: 0 }}>{title}</p>
            ...
            <button
              ...
              style={{
                ...
                background: "rgba(255,255,255,0.07)", color: C.muted, cursor: "pointer",
...
export default function HirerPromotions() {
  ...
  useEffect(() => {
    ...
    hirerAPI
      ...
      .catch((e) => { if (m) setError(e.message || "Failed to load promotions"); })
  ...
  const handleCreatePromotion = async (form) => {
    const created = await hirerAPI.createPromotion({
      ...
      type: "featured",
      duration: form.deadline
        ? Math.max(1, Math.ceil((new Date(form.deadline).getTime() - Date.now()) / 86400000))
        : 7,
      price: Number(form.reward || 0),
    });
    const mapped = {
      ...
      requirements: [],
      createdBy: "Artlancerr",
      acceptedCount: 0, submittedCount: 0, approvedCount: 0,
      rejectedCount: 0, pendingReview: 0,
      status: "Active",
      submissions: [],
    };
    setAllPromos((prev) => [mapped, ...prev]);
    ...
    setMyPromos((prev) =>
      prev.map((p) =>
        p.id !== promoId ? p : {
          ...
          pendingReview: Math.max(0, p.pendingReview - 1),
          approvedCount: p.approvedCount + 1,
          submissions: p.submissions.map((s) => s.id === subId ? { ...s, status: "Approved" } : s),
        },
      ),
    );
    ...
    setMyPromos((prev) =>
      prev.map((p) =>
        p.id !== promoId ? p : {
          ...
          pendingReview: Math.max(0, p.pendingReview - 1),
          rejectedCount: p.rejectedCount + 1,
          submissions: p.submissions.map((s) => s.id === subId ? { ...s, status: "Rejected" } : s),
        },
      ),
    );
  ...
  return (
    <div
      className="min-h-screen flex"
      style={{ background: C.bg, fontFamily: "'Plus Jakarta Sans', sans-serif" }}
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
        .pc { transition: border-color 0.2s, box-shadow 0.2s; }
      ...
      {/* ── HirerSidebar: handles its own mobile toggle internally ── */}
      <HirerSidebar />
      ...
      <div className="flex-1 min-w-0 lg:ml-72">
        <div className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8 max-w-5xl mx-auto">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl font-semibold" style={{ color: C.text, marginBottom: 2 }}>
                Promotions
              </h1>
            ...
            <button
              onClick={() => setShowCreate(true)}
              className="flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-lg text-sm font-semibold outline-none border-0 cursor-pointer flex-shrink-0"
              style={{ background: `linear-gradient(135deg, ${C.gold}, #d4b56e)`, color: "#1a1d24" }}
          ...
          <div className="flex mb-6" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            {[["all", "All Promotions"], ["my", "My Promotions"]].map(([key, label]) => (
              <button
                key={key}
                onClick={() => setTab(key)}
                className="tln px-4 sm:px-6 py-3 text-sm font-medium outline-none border-0 bg-transparent cursor-pointer"
          ...
          <AnimatePresence mode="wait">
                ...
                {(allPromos || []).map((p, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={{ delay: i * 0.06 }}
                      className="pc flex overflow-hidden rounded-xl"
                      style={{ background: C.card, border: `1px solid ${C.border}`, opacity: isFull || expired ? 0.6 : 1 }}
                    >
                      {/* Poster */}
                      <div className="relative flex-shrink-0 w-28 sm:w-36 md:w-44" style={{ minHeight: 190 }}>
                        <Img src={p.poster} alt={p.projectName} className="absolute inset-0 w-full h-full" />
                        {(isFull || expired) && (
                          <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.55)" }}>
                            <span style={{ color: "#fff", fontWeight: 700, letterSpacing: 2, fontSize: 11 }}>
                              {isFull ? "FULL" : "EXPIRED"}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="flex-1 p-3 sm:p-5 lg:p-6 flex flex-col justify-between min-w-0">
                        <div>
                          <h3 className="text-base sm:text-lg font-semibold mb-2" style={{ color: C.text }}>{p.projectName}</h3>
                          <div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm mb-3">
                            <span className="flex items-center gap-1" style={{ color: C.muted }}><Instagram size={13} /> {p.promotionType}</span>
                            <span className="flex items-center gap-1 font-semibold" style={{ color: C.gold }}><DollarSign size={13} /> {p.reward} reward</span>
                            <span className="flex items-center gap-1" style={{ color: C.muted }}><Users size={13} /> {rem}/{p.totalSlots} slots</span>
                            <span className="flex items-center gap-1" style={{ color: expired ? C.red : C.muted }}><Clock size={13} /> {tl}</span>
                          </div>
                          <p
                            className="text-xs sm:text-sm leading-relaxed"
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <span style={{ color: C.muted, fontSize: 12 }}>By {p.createdBy}</span>
                          <button
                            onClick={() => setDetailPromo(p)}
                            className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium outline-none cursor-pointer"
            ...
            {tab === "my" && (
              ...
              >
                {myPromos.map((p, i) => {
                  const tl = timeLeft(p.deadline, now);
                  const pct = Math.round((p.acceptedCount / p.totalSlots) * 100);
                  return (
                    <motion.div
                      key={p.id}
                      initial={{ opacity: 0, y: 16 }}
                    ...
                    >
                      {/* Poster */}
                      <div className="relative flex-shrink-0 w-28 sm:w-36 md:w-44" style={{ minHeight: 260 }}>
                        <Img src={p.poster} alt={p.projectName} className="absolute inset-0 w-full h-full" />
                      ...
                      <div className="flex-1 p-3 sm:p-5 lg:p-6 flex flex-col gap-3 sm:gap-4 min-w-0">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm">
                            <span className="flex items-center gap-1" style={{ color: C.muted }}><Instagram size={13} /> {p.promotionType}</span>
                            <span className="flex items-center gap-1 font-semibold" style={{ color: C.gold }}><DollarSign size={13} /> {p.reward} per artist</span>
                            {p.status === "Active" && tl !== "Expired" && (
                              <span className="flex items-center gap-1" style={{ color: C.muted }}><Clock size={13} /> {tl} left</span>
                            )}
                          </div>
                        </div>
                        {/* Progress */}
                        <div>
                          <div className="flex justify-between text-xs sm:text-sm mb-1.5">
                            <span style={{ color: C.muted }}>Participation</span>
                            <span className="font-semibold" style={{ color: C.text }}>{p.acceptedCount} / {p.totalSlots}</span>
                          </div>
                          <ProgressBar pct={pct} />
                        </div>
                        {/* Stats */}
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          {[
                            { label: "Submitted", val: p.submittedCount, bg: C.cardInner, col: C.text },
                            { label: "Approved", val: p.approvedCount, bg: C.greenDim, col: C.green },
                            { label: "Pending", val: p.pendingReview, bg: C.yellowDim, col: C.yellow },
                            { label: "Rejected", val: p.rejectedCount, bg: C.redDim, col: C.red },
                          ].map(({ label, val, bg, col }) => (
                            <div key={label} className="rounded-lg p-2 sm:p-3 text-center" style={{ background: bg }}>
                              <p style={{ color: col, fontSize: 20, fontWeight: 600, margin: 0 }}>{val}</p>
                              <p style={{ color: C.muted, fontSize: 11, marginTop: 2 }}>{label}</p>
                            </div>
                          ))}
                        </div>
                        {/* Actions */}
                        <div className="flex flex-col sm:flex-row gap-2">
                          {p.pendingReview > 0 && (
                            <button
                              ...
                              onMouseLeave={(e) => { e.currentTarget.style.filter = "none"; }}
                            >
                              <AlertCircle size={14} /> Review Submissions ({p.pendingReview})
                            </button>
                          )}
                          <button
                            onClick={() => setReviewPromo(p)}
      ...
      {/* ── Promotion Details Modal ── */}
      <Modal open={!!detailPromo} onClose={() => setDetailPromo(null)} title="Promotion Details" subtitle="View the full details of this promotion">
        {detailPromo && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <div style={{ width: 96, height: 144, borderRadius: 12, overflow: "hidden", flexShrink: 0 }}>
                <Img src={detailPromo.poster} alt={detailPromo.projectName} className="w-full h-full" />
              </div>
              <div style={{ flex: 1, minWidth: 160 }}>
                <p style={{ color: C.text, fontSize: 18, fontWeight: 600, marginBottom: 10 }}>{detailPromo.projectName}</p>
                {[
                  { Icon: Instagram, label: detailPromo.promotionType, color: C.muted },
                  { Icon: DollarSign, label: `${detailPromo.reward} fixed reward`, color: C.gold },
                  { Icon: Calendar, label: `Deadline: ${new Date(detailPromo.deadline).toLocaleDateString()}`, color: C.muted },
                  { Icon: Users, label: `${detailPromo.totalSlots} total slots`, color: C.muted },
                ].map(({ Icon, label, color }, idx) => (
                  <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, color, fontSize: 13, marginBottom: 6 }}>
                    <Icon size={14} /> {label}
                  </div>
                ))}
              </div>
            ...
            <div style={{ height: 1, background: C.borderSub }} />
            <div>
              <p style={{ color: C.text, fontWeight: 600, marginBottom: 6 }}>Description</p>
              <p style={{ color: C.muted, fontSize: 13, lineHeight: 1.6 }}>{detailPromo.description}</p>
            </div>
            {detailPromo.requirements?.length > 0 && (
              <div>
                <p style={{ color: C.text, fontWeight: 600, marginBottom: 10 }}>Requirements</p>
                {detailPromo.requirements.map((r, idx) => (
                  <div key={idx} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 8 }}>
                    <CheckCircle size={16} style={{ color: C.gold, flexShrink: 0, marginTop: 1 }} />
                    <span style={{ color: C.muted, fontSize: 13 }}>{r}</span>
                  </div>
                ))}
              </div>
            ...
            <button
              onClick={() => setDetailPromo(null)}
              style={{ width: "100%", padding: "13px", borderRadius: 12, border: "none", background: `linear-gradient(135deg, ${C.gold}, #d4b56e)`, color: "#1a1d24", fontSize: 14, fontWeight: 600, cursor: "pointer", marginTop: 4 }}
              onMouseEnter={(e) => { e.currentTarget.style.filter = "brightness(1.08)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.filter = "none"; }}
            >
              Close
      ...
      {/* ── Review Submissions Modal ── */}
      <Modal open={!!reviewPromo} onClose={() => setReviewPromo(null)} title="Review Submissions" subtitle="Approve or reject artist submissions" wide>
        {reviewPromo && (
          reviewPromo.submissions.length === 0
            ? <p style={{ color: C.muted, textAlign: "center", padding: "40px 0" }}>No submissions yet.</p>
            : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {reviewPromo.submissions.map((sub) => (
                  <motion.div
                    key={sub.id}
                    layout
                    style={{ background: C.cardInner, border: `1px solid ${C.borderSub}`, borderRadius: 12, padding: 16 }}
                  >
                    <div style={{ display: "flex", gap: 14 }}>
                      <div style={{ width: 48, height: 48, borderRadius: "50%", overflow: "hidden", flexShrink: 0 }}>
                        <Img src={sub.artistPhoto} alt={sub.artistName} className="w-full h-full" />
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 3, flexWrap: "wrap", gap: 8 }}>
                          <p style={{ color: C.text, fontWeight: 600, margin: 0 }}>{sub.artistName}</p>
                          <StatusBadge status={sub.status} />
                        </div>
                        <p style={{ color: C.muted, fontSize: 12, marginBottom: 8 }}>
                          Submitted {new Date(sub.submittedAt).toLocaleDateString("en-US", { month: "numeric", day: "numeric", year: "numeric" })}
                        </p>
                        <a href={sub.proofUrl} target="_blank" rel="noopener noreferrer" style={{ color: C.gold, fontSize: 13, display: "block", marginBottom: 10, wordBreak: "break-all" }}>
                          {sub.proofUrl}
                        </a>
                        {sub.status === "Pending" && (
                          <div style={{ display: "flex", gap: 8 }}>
                            <button
                              onClick={() => approve(reviewPromo.id, sub.id)}
                              style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 0", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${C.gold}, #d4b56e)`, color: "#1a1d24", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
                              onMouseEnter={(e) => { e.currentTarget.style.filter = "brightness(1.08)"; }}
                              onMouseLeave={(e) => { e.currentTarget.style.filter = "none"; }}
                            >
                              <ThumbsUp size={14} /> Approve
                            ...
                            <button
                              onClick={() => reject(reviewPromo.id, sub.id)}
                              style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 0", borderRadius: 10, border: `1px solid rgba(248,113,113,0.25)`, background: C.redDim, color: C.red, fontSize: 13, fontWeight: 600, cursor: "pointer" }}
      ...
      {/* ── Create Promotion Modal ── */}
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create Promotion" subtitle="Set up a new promotion mission for artists">
        <CreateForm onClose={() => setShowCreate(false)} onCreate={handleCreatePromotion} />
      </Modal>
    </div>
  );
}
...
function CreateForm({ onClose, onCreate }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div>
        <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Project Name</label>
        <FieldInput value={form.projectName} onChange={(e) => set("projectName", e.target.value)} placeholder="Enter movie/project name" />
      </div>
      <div>
        <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Description</label>
        <textarea
          value={form.description}
          onChange={(e) => set("description", e.target.value)}
          placeholder="Describe what participants need to do..."
          rows={4}
          style={{ width: "100%", borderRadius: 12, background: C.inputBg, border: `1px solid ${C.inputBorder}`, color: C.text, padding: "10px 14px", fontSize: 14, fontFamily: "'Plus Jakarta Sans', sans-serif", outline: "none", resize: "none", boxSizing: "border-box" }}
          onFocus={(e) => { e.target.style.borderColor = "rgba(201,169,97,0.5)"; }}
          onBlur={(e) => { e.target.style.borderColor = C.inputBorder; }}
        />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div>
          <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Promotion Type</label>
          <div style={{ position: "relative" }}>
            <select
              value={form.promotionType}
              onChange={(e) => set("promotionType", e.target.value)}
              style={{ width: "100%", borderRadius: 12, background: C.inputBg, border: `1px solid ${C.inputBorder}`, color: form.promotionType ? C.text : C.muted, padding: "10px 36px 10px 14px", fontSize: 14, fontFamily: "'Plus Jakarta Sans', sans-serif", outline: "none", appearance: "none", cursor: "pointer" }}
              onFocus={(e) => { e.target.style.borderColor = "rgba(201,169,97,0.5)"; }}
              onBlur={(e) => { e.target.style.borderColor = C.inputBorder; }}
            >
              <option value="" disabled style={{ background: "#22252e" }}>Select type</option>
              <option value="story" style={{ background: "#22252e" }}>Instagram Story</option>
              <option value="post" style={{ background: "#22252e" }}>Instagram Post</option>
              <option value="reel" style={{ background: "#22252e" }}>Instagram Reel</option>
            </select>
            <svg style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: C.muted }} width="11" height="11" viewBox="0 0 12 12" fill="currentColor">
              <path d="M6 8L1 3h10z" />
            </svg>
          </div>
        </div>
        <div>
          <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Fixed Reward (per participant)</label>
          <FieldInput type="number" value={form.reward} onChange={(e) => set("reward", e.target.value)} placeholder="25" />
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div>
          <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Total Slots</label>
          <FieldInput type="number" value={form.totalSlots} onChange={(e) => set("totalSlots", e.target.value)} placeholder="50" />
        </div>
        <div>
          <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Deadline</label>
          <FieldInput type="datetime-local" value={form.deadline} onChange={(e) => set("deadline", e.target.value)} />
        </div>
      </div>
      <div>
        <label style={{ color: C.text, fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>Movie Poster</label>
        <button
          style={{ width: "100%", padding: "32px 0", borderRadius: 12, border: "2px dashed rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.02)", color: C.muted, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}
          onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.gold; e.currentTarget.style.color = C.gold; }}
          onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.12)"; e.currentTarget.style.color = C.muted; }}
        >
          <Upload size={28} strokeWidth={1.5} />
          <span style={{ fontSize: 14, fontWeight: 500 }}>Upload movie poster</span>
          <span style={{ fontSize: 12, opacity: 0.7 }}>Recommended: 2:3 aspect ratio</span>
        </button>
      </div>
      <div style={{ display: "flex", gap: 12 }}>
        <button
          onClick={async () => { await onCreate?.(form); onClose(); }}
          style={{ flex: 1, padding: "12px 0", borderRadius: 12, border: "none", background: `linear-gradient(135deg, ${C.gold}, #d4b56e)`, color: "#1a1d24", fontSize: 14, fontWeight: 600, cursor: "pointer" }}
          onMouseEnter={(e) => { e.currentTarget.style.filter = "brightness(1.08)"; }}
          onMouseLeave={(e) => { e.currentTarget.style.filter = "none"; }}
        >
          Create Promotion
        ...
        <button
          onClick={onClose}
          style={{ flex: 1, padding: "12px 0", borderRadius: 12, border: `1px solid ${C.border}`, background: "transparent", color: C.text, fontSize: 14, fontWeight: 600, cursor: "pointer" }}
